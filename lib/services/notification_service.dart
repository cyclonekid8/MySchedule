...
class NotificationService {
  ...
  Future<void> _createNotificationChannel() async {
    try {
      ...
      // Try to verify channel creation (graceful fallback for compatibility)
      try {
        final channels = await android.getNotificationChannels();
        final created = channels?.any((c) => c.id == 'activity_reminders') ?? false;
        _showFeedback(created ? '✅ Channel created and verified' : '✅ Channel created (verification inconclusive)');
      } catch (verifyError) {
        _showFeedback('✅ Channel created (verification skipped - device compatibility)');
      }
    } catch (e) {
  ...
  Future<bool> canScheduleExactAlarms() async {
    try {
      ...
      _showFeedback('Exact alarm check failed, assuming allowed: $e');
      return true; // Assume allowed on older versions
    }
  }
  ...
  Future<void> requestExactAlarmPermission() async {
    try {
      ...
      _showFeedback('Exact alarm permission request failed: $e');
    }
  }
  ...
  Future<void> scheduleActivityReminder(Activity activity) async {
    ...
    try {
      ...
      // Always ensure channel exists before scheduling (with fallback)
      try {
        final channels = await android.getNotificationChannels();
        ...
        // If we can't check channels, just try to create it anyway
